## nansen-pulakat

First time:

1. Make sure `git` is installed on your machine. If it is not, on Windows, go [here](https://git-scm.com/download/win). On Mac, open a terminal, and type `xcode-select --install`. Accept the license and wait for install. On Linux, consult your Linux distribution's package manager.
2. Download the file [(https://img.shields.io/badge/_-install.m-blue?logo=github)](https://github.com/Waltham-Data-Science/nansen-pulakat/raw/main/install.m). Note: If the link opens as text in your browser, simply press Ctrl+S (or Cmd+S) and save it to your computer as `install.m`.
3. Run `install.m` in MATLAB.

Subsequent uses:

Run `pulakat.setup` to open the GUI.