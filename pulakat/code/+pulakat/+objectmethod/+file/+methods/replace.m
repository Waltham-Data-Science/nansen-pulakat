function varargout = replace(fileObject, varargin)
%REPLACE Replaces metadata values for selected files.
%
%   This object method allows the user to choose a column and replace
%   an old value with a new value for all selected files.
%
%   Inputs:
...
%       varargout: If called without inputs, returns the method's attributes.
    ...
    availableVars = fileTable.Properties.VariableNames;
    ...
    % Get identifying columns for files
    labName = project.Name;
    projectFile = fullfile('+ndi','+setup','+conv',['+',labName],'project_info.json');
    projectInfo = jsondecode(fileread(projectFile));
    identifyingVars = [{'SessionName', 'ElectronicFileName', 'DataTypeName'}, projectInfo.subjectIdentifierFields'];

    identifyingTable = fileTable(:, identifyingVars);

    % Call backend replace function
    ndi.nansen.metatable.replace('file', columnName, oldValue, newValue, ...
