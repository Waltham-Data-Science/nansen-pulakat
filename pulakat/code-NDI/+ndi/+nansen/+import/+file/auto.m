...
dataTable_new = ndi.nansen.fun.editImportTableGUI(dataTable_type(indNew,:), ...
    'DropDown', ddOpts, 'Editable', 'DataTypeName', 'AddRow', false,...
...
for i = 1:numel(fileTypes)
    ...
    subjectTable_type{i} = ndi.nansen.fun.editImportTableGUI(thisTable, ...
        ...
        'Prompt',sprintf(['Confirm the subject information for each new %s file to import. ' ...
        'Ensure that each subject has a fully unique set of identifiers. ' ...
        'File(s) will not be imported if all matching subject rows are deleted.'],...
        fileTypes{i}));
end
