...
for i = 1:height(dataTable)
    ...
    if numMatch(i) > 1
        warning('NDI:Nansen:Import:File:AmbiguousSubject', ...
            ['[NDI:Nansen:Import:File:AmbiguousSubject] More than one ' ...
             'subject matches file ''%s''. Using first match; consult ' ...
             'NDI to resolve.'], dataTable.ElectronicFileName{i});
        ind = ind(1);
    elseif numMatch(i) == 0
        continue
    end
...
if any(numMatch == 0)
    ...
    warning('NDI:Nansen:Import:File:NoSubjectMatch', ...
        ['[NDI:Nansen:Import:File:NoSubjectMatch] %d file(s) skipped ' ...
         'because no subject matched. Import the missing subjects first ' ...
         'and retry:\n  %s'], ...
