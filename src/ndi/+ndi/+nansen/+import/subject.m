...
if isempty(subjectTable_session)
    ...
    for i = 1:height(subjectTable)
        for k = 1:numMatch(i)
            ...
            if any(conflictMask)
                warning('NDI:Nansen:Import:Subject:Conflict', ...
                    ['[NDI:Nansen:Import:Subject:Conflict] Conflict found ' ...
                     'for subject matching %s in variables: %s. Existing ' ...
                     'data will be preserved.'], ...
                    strjoin(cellstr(string(A(1, 1:min(3, end)))), ' '), ...
                    strjoin(commonVars(conflictMask), ', '));
            end
