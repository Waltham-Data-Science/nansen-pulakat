...
if nnz(allDataSheetInd) ~= 1
    error('NDI:Nansen:Setup:Conv:Pulakat:DIA:SheetNotFound', ...
        ['[NDI:Nansen:Setup:Conv:Pulakat:DIA:SheetNotFound] Expected ' ...
         'exactly one sheet containing ''All data'' in %s, found %d.\n' ...
         'Sheets present: %s'], ...
...
for j = 1:numel(diaSubjectVars)
    ...
    if isempty(tokens)
        warning('NDI:Nansen:Setup:Conv:Pulakat:DIA:UnparsedColumn', ...
            ['[NDI:Nansen:Setup:Conv:Pulakat:DIA:UnparsedColumn] ' ...
             'Skipping DIA column ''%s'' that does not match the subject ' ...
             'pattern.'], diaSubjectVars{j});
        continue
    end
    if ~isempty(tokens{4})
